...
'AllP_raw' contains all *p*-values extracted by the R statcheck package (Epskamp and Nuijten 2016) from the 2014-2016 volumes of *ASR*, *AJS*, *SQ*, *JMF*, and *CHQ*. To improve readability, we created the Excel-file 'AllP'. Here, information on article, author(s), and year was split into different columns, and some other relevant information was manually added. Variables created by statcheck are indicated below by a boldfaced variable name. The variables in 'AllP' are: 
...
10. Reported.Not.Relevant: a score of '1' = relevant and '0' = not relevant. In the following cases, extracted results which were reported as being p-values were not deemed relevant:
    ...
    - an 'extracted *p*-value' was actually not a *p*-value, but another kind of value.
    - an extracted *p*-value was a duplicate of another extracted *p*-value.
    - There was one case in which the *p*-value was reported as 'Ns' and 'Ns' did not mean non-significant, but meant 'numbers'.  
11.	Not.Reported: contains information on whether an extracted result could (not) be linked 1 to 1 to a result from the article, where '1' = no and '0' = yes. If an extracted result scored '0', it was excluded from analyses.
12. Model.fit: indicates whether an extracted *p*-value belongs to a model fit statistic or not, where '1' = yes and 'NA' = no.
...
16. Explicitly stated hypothesis: contains the phrasing of the explicitly stated hypothesis, or 'NA' when there was no explicitly stated hypothesis.
