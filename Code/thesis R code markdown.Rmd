...
```{r tables on errors (APA), warning=FALSE, message = FALSE, echo = TRUE, results = 'hide'}
#Errors and journals
...
Creating relevant subsets and recalculating *p*-values for results with *r*-values and *df*. For this purpose, *t*-values first have to be calculated, which can then be used in the pt() formula to calculate the *p*-values corresponding with bivariate correlations.
...
for (i in 1:nrow_Hyp_r){
  {Hyp_r$t_stat[i] <- (Hyp_r$b[i] * sqrt(Hyp_r$df_1[i])) / sqrt(1-Hyp_r$b[i]^2)
...
```
Merging the data sets containing results with reported *r*-values and *df*.
```{r merging data sets of results with r-values, warning=FALSE, message = FALSE, echo = TRUE, results = 'hide'}
#Merging data sets with results that have reported comparisons '=' and '<'.
Hyp_r_mrgd <- merge(Hyp_r_sml, Hyp_r_ns, all = TRUE) #merging 
nrow(as.data.frame(Hyp_r_mrgd)) #n = 48
...
nrow(Hyp_fin) #n = 399
mean(as.numeric(Hyp_fin$Error))*100 #6.3% 
mean(as.numeric(Hyp_fin$DecisionError))*100 #1.3%
