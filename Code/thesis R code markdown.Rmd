...
Software version used: Rstudio Version 1.3.959.

# 1. Preliminary procedures

Loading packages used later on in the code.
```{r loading packages, warning=FALSE, message = FALSE}
#checking for dependencies
#install.packages("dependencies")
#statcheck is used to retrieve p-values and fully APA-reported results from the text of articles
library("statcheck") 
...
Checking for dependencies (change chunk to eval = TRUE to view results in .Rmd file).
```{r checking dependencies, warning=FALSE, message = FALSE, eval = FALSE}
#checking for dependencies
renv::dependencies()
```
\

Since we found that for *JMF*, statcheck did not succeed in extracting results with a '-' sign. After contacting the author, Mich&#232;le Nuijten, about this problem, she adapted the GitHub Version of the package such that this problem would be solved. Below, the GitHub version of the package is loaded into R. 
...
Extracting *p*-values and fully APA-reported results from all collected articles. Initially, we accidentally left out articles from the 2015 volume of *CHQ*, so results from this volume were retrieved separately. Then, all retrieved results were merged. Numbers of rows of *p*-values and fully APA-reported results were 7,280 and 524, respectively.
```{r extracting p-values and fully APA-reported results from articles, warning=FALSE, message = FALSE, eval = FALSE}
#All articles except those from CHQ 2015
jrnls_tot <- "C:/Users/Elise/Documents/Master's thesis/Literature/Articles/All journals_05_02" #file with all articles, minus those from CHQ 2015
outp_jrnl_tot <- checkHTMLdir(jrnls_tot, extension = TRUE, AllPValues = TRUE) #extracting p-values
outp_jrnl_tot_sre <- checkHTMLdir(jrnls_tot, extension = TRUE) #extracting fully APA-reported results 

#Articles CHQ, volume 2015. 
jrnls_extra <- "C:/Users/Elise/Documents/Master's thesis/Literature/Articles/Extra journals" #file with all articles 2015 volume CHQ
outp_jrnl_extra <- checkHTMLdir(jrnls_extra, extension = TRUE, AllPValues = TRUE) #extracting p-values 
outp_jrnl_extra_sre <- checkHTMLdir(jrnls_extra, extension = TRUE) #extracting fully APA-reported results

#Constructing final data sets by merging retrieved data from CHQ year 2015 with the other data.
merge_outp_p <- merge(outp_jrnl_tot, outp_jrnl_extra, all=TRUE) #merging retrieved p-values
nrow(merge_outp_p) #n = 7280 retrieved p-values in total
merge_outp_sre <- merge(outp_jrnl_tot_sre, outp_jrnl_extra_sre, all=TRUE) #merging fully APA-reported retrieved results
nrow(merge_outp_sre) #n = 524 fully APA-reported retrieved results in total
```
\ 

Writing extracted data to text files. These will be saved in a path where all the other data sets and code can also be found.
```{r writing extracted data to text files, warning=FALSE, message = FALSE, eval = FALSE}
#Creating a .txt file with the information obtained by statcheck retrieving p-values.
write.table(outp_jrnl_tot, "C:/Users/Elise/Documents/GitHub/thesis/Data/AllP/AllP_raw.txt", sep = ",")
#Creating a .txt file with the information obtained by statcheck on fully APA-reported results.
write.table(outp_jrnl_tot_sre, "C:/Users/Elise/Documents/GitHub/thesis/Data/APA/APA_raw.txt", sep = ",")
```
\
\
\
...
Reading the Excel-file containing information obtained when extracting fully APA-reported results using statcheck and additional, manually added information (*n* = 505 from 76 articles). 
...
APA_u <- APA[!duplicated(APA$Article_numb), ] #removing duplicates to obtain number of articles
nrow(APA_u) #n = 80
options(scipen = 999)
...
Running a logistic regression with the variable indicating whether a result is an error or not as the outcome variable and a predictor indicating whether a result belongs to an explicitly stated hypothesis or not. There does not seem to be a significant difference in the amount of inconsistencies between results related to hypotheses and results not related to hypotheses, *b* = -.073, SE = .278, *z* = -.262, *p* = .793, OR(inverse) = 1.076. Thus, we cannot confirm H1. 
```{r testing H1, warning=FALSE, message = FALSE, echo = TRUE, results = 'hide'}
APA$Error_n <- as.numeric(as.character(APA$Error)) #making 'Error' numeric
...
Running a logistic regression with the variable indicating whether a result is a decision error or not as the outcome variable and a predictor indicating whether a results belongs to an explicitly stated hypothesis or not as a predictor. There does not seem to be a significant difference in the amount of grossly inconsistent results between results related to hypotheses and results not related to hypotheses, *b* = .708, SE = .714, *z* = .993, *p* = .321, OR = 2.030. Therefore, we cannot confirm H2.  
...
# 3. Data set 'AllP'
Reading the Excel-file with both the information obtained when extracting *p*-values using statcheck and information that was added manually in Excel. 
...
### 3.1.3  *Statistical analysis p-value range (.04-.06] (H3a)*
Logistic regression to study the difference in the prevalence of publication bias between results related to explicitly stated hypotheses and results not related to explicitly stated hypotheses for binwidth .01. The prevalence of just significant and just non-significant results does not seem to differ significantly for results related to hypotheses and results not related to hypotheses, b1 = -.132, SE = .504, *p* = .794, *z* = -.261, OR = .877, 95% CI [.321, 2.345]. Thus, we could not confirm H3a.
...
### 3.1.4  *Statistical analysis p-value range (.03-.07] (H3b)*
Logistic regression to study the difference in the prevalence of publication bias between results related to explicitly stated hypotheses and results not related to explicitly stated hypotheses for binwidth .02. There does not seem to be a significant difference in the prevalence of just significant results among results related to hypotheses and results not related to hypotheses, b1 = -.251, SE = .392, *p* = .521, *z* = -.642, OR = .778, 95% CI [.358, 1.674]. Thus, we could not confirm H3b.
...
## 3.3 Marginal significance

### *3.3.1 Preliminary calculations*
Firstly, we calculate some basic statistics relevant to marginal significance at the article and results level. These will be used to calculate other statistics later on. Results that are reported using the sign ‘>’ are not taken into account, since this do not imply results were near the conventional significance level, i.e., *p* = .05. 
...
AllP_marg <- subset(AllP, Reported.P.Value > .05 & Reported.P.Value <= .1 & Reported.Comparison < 5) #subsetting the data, no results with '>'
AllP_marg_n <- nrow(AllP_marg) 
AllP_marg_n #n = 206 results with p-values in range (.05-.10]

#Subset marginally significant results.
AllP_marg_1 <- subset(AllP_marg, Marg.Sig == 1) 
AllP_marg_1_n <- nrow(AllP_marg_1) 
AllP_marg_1_n #n = 72 results reported as marginally significant
AllP_marg_perc <- round(((AllP_marg_1_n/AllP_marg_n)*100), digits = 1)
...
AllP_marg_1_nd_nrow <- nrow(AllP_marg_1_nd) 
AllP_marg_1_nd_nrow #n = 46 

#Removing duplicates to count number of articles.
AllP_marg_nd <- AllP_marg[!duplicated(AllP_marg$Article_numb), ] 
AllP_marg_nd_tot <- nrow(AllP_marg_nd) 
AllP_marg_nd_tot #n = 107 articles with p-values in range (.05-.10]
AllP_marg_nd_perc <- round((AllP_marg_1_nd_nrow/AllP_marg_nd_tot)*100, digits = 1) #percentage

#Subset results that are reported as not marginally significant.
AllP_marg_0 <- subset(AllP_marg, Marg.Sig == 0)
AllP_marg_0_n <- nrow(AllP_marg_0) 
AllP_marg_0_n #n = 134

#Articles with no p-values in the range (.05-.10] that are reported as marginally significant.
AllP_marg_0_nd_nrow <- AllP_marg_nd_tot - AllP_marg_1_nd_nrow 
AllP_marg_0_nd_nrow #61
```
\
\

### *3.3.2 Descriptive information article level*

Creating tables containing information on marginal significance at the article level. Data on different categories are included.
...
AllP_jrnl_marg_0_nd <- AllP_jrnl_marg_tot_nd - AllP_jrnl_marg_1_nd #no results reported as marginally significant
...
AllP_yr_marg_0_nd <- AllP_yr_marg_tot_nd - AllP_yr_marg_1_nd #no results reported as marginally significant
...
### *3.3.3 Descriptive information results level*  

Creating tables containing information on marginal significance at the results level. Data for different categories are included.
...
### *3.3.4 Final tables on marginal significance*
...
###  *3.3.5 Statistical analysis marginal significance (H4)*
Logistic regression to study the difference in the prevalence of marginally significant results between results related to explicitly stated hypotheses and results not related to explicitly stated hypotheses. The prevalence of marginally significant results does not seem to differ significantly for results related to hypotheses and results not related to hypotheses. Thus, we cannot confirm H4, b1 = -0.437, SE = .318, *z* = -1.373, *p* = .170, OR = .646, 95% CI [.342, 1.194].
...
The 2014-2016 volumes of *ASR*, *AJS*, and *SQ* contained 322 articles. After our selection procedure, 93 articles were used in our final data set 'Hyp'.
...
Hyp_art_hyp1 <- subset(Hyp_art, Hypotheses == 1)
...
Hyp_art_hyp1_stat <- subset(Hyp_art_hyp1, Req_stat == 1) 
...
Hyp <- read_excel("./Hyp & inclusion_Hyp/Hyp.xlsx", col_types = c(
  ...
  #dec_pval_rep, Computed, P_one_sided, Error
  "numeric", "numeric", "numeric", "numeric", 
...
#Final subset of results with regression coefficients and/or t-statistics. This includes only those results for which df were reported. 
...
#Final subset of results with chi2-statistics, which contains only results for which the df are known. This includes two results from Doan, Loehr & Miller (2014) for which the chi2-statistic was reported as '.001'in the 'Hyp' Excel file, but for which the actual reported value was '< .001' (see 'Hyp' Excel file, column 'b'). The p-values assigned to these unequally reported statistics, which are reported as 'ns', can be recalculated despite the chi2-statistics being unequally reported. Since the lower a chi2-statistic is, the less significant it becomes, we could test whether 'ns' is accurate by filling in .001 as the p-value in the cases described above. 
...
### <u> Important note on recalculations, read this first for clarity. </u>
In this section, *p*-values from 'Hyp' will be recalculated using information on different statistics. The order of statistics recalculated is as follows: *t*,  *r*, *z*, $\chi^2$, and *F*. During recalculation, we ran into two issues:\   
1. We had cases in which OneSided = 0 (meaning it was unclear whether one-sided or two-sided testing was used) & Computed > 1 under our initial (working) assumption that two-sided testing was used in cases with OneSided = 0. For these results, OneSided = 0 was immediately converted to OneSided = 1. A new *p*-value was determined immediately after, as was the correctness of this new *p*-value.\   
2. We had cases in which results with OneSided = 0 turned out to be erroneous under two-sided testing, but without leading to Computed > 1. Since Computed > 1 is always incorrect, but results that seemed to be erroneous under two-sided testing with Computed $\le$ 1 did have theoretically possible *p*-values, we decided to recalculate this category of results after doing all of our initial calculations. Thus, after recalculating results for the different statistics (*t*,  *r*, *z*, $\chi^2$, and *F*), these results were assigned OneSided == 1 and recalculated as such. The results following from these recalculations were incorporated into the final results. Thus, if the outcome of one-sided testing was that a result did turn out to be correct, we assumed that was correct, and that authors had just failed to mention one-sided testing in the article in question. If a result was still incorrect after one-sided testing, we assumed a result was definitely incorrect.


### *4.3.1 Recalculating p-values based on t-values*

*No (decision) errors were found among reported p-values based on t-values, see below.*
\
...
nrow_Hyp_t_df <- nrow(Hyp_t_df) #object number of columns 
...
#Numbers of (decision) errors.
sum(as.numeric(Hyp_t_sml$Error)) #n = 0
sum(as.numeric(Hyp_t_sml$DecisionError)) #n = 0
```
\

Function that calculates the numbers of decimal places (credits to daroczig and owen88, see https://stackoverflow.com/questions/5173692/how-to-return-number-of-decimal-places-in-r). In our case, this function will be used on exactly reported *p*-values, such that we can obtain rounded *p*-values which indicate the lowest and highest reported *p*-values that would be acceptable when taking potential rounding errors into account. Note that this function rounds off to the smallest non-zero value of the reported *p*-value. E.g., *p* = 0.0000 will have 0 decimals, and *p* = .0010 will have 3 decimals.
...
Determining whether there are (decision) errors among results with exactly reported *p*-values for which regression coefficients and standard errors were not reported, but *t*-statistics were reported. 
...
Hyp_eq_t <- subset(Hyp_t_df, Reported.Comparison == 4) #subset results with exactly reported p-values
nrow(Hyp_eq_t) #n = 7
nrow_Hyp_eq_t <- nrow(Hyp_eq_t) #object number of columns 
...
#Numbers of (decision) errors.
sum(as.numeric(Hyp_eq_t$Error)) #0
sum(as.numeric(Hyp_eq_t$DecisionError)) #0
```
\
\
...
*No (decision) errors were found among reported p-values based on r-values, see below.*
\

Creating relevant subsets and recalculating *p*-values for results with *r*-values and *df*. For this purpose, *t*-values first have to be calculated. These can then be used in the pt() formula to calculate *p*-values corresponding to bivariate correlations.
```{r recalculating p-values with '<' based on r-statistics, warning=FALSE, message = FALSE, echo = TRUE, results = 'hide'}
nrow_Hyp_r <- nrow(Hyp_r) #object number of columns 
...
#Numbers of (decision) errors.
sum(as.numeric(Hyp_r_sml$Error)) #n = 0
sum(as.numeric(Hyp_r_sml$DecisionError)) #n = 0
```
\
...
#Since a (decision) error occurs if a result that was reported as non-significant is significant, a "1" is assigned if Computed <= .05, and a "0" is assigned if Computed > .05.
...
Hyp_eq_r <- subset(Hyp_r, Reported.Comparison == 4) #subset results with exactly reported p-values
nrow(Hyp_eq_r) #n = 4
nrow_Hyp_eq_r <- nrow(Hyp_eq_r) #object number of columns 
...
#Numbers of (decision) errors.
sum(as.numeric(Hyp_eq_r$Error)) #0
sum(as.numeric(Hyp_eq_r$DecisionError)) #0
```
\
\

### *4.3.3 Recalculating p-values based on z-values* 

*Among reported p-values based on z-values, 12 (decision) errors were found, see below.*
...
#Numbers of (decision) errors.
sum(as.numeric(Hyp_z_ns$Error)) #0
sum(as.numeric(Hyp_z_ns$DecisionError)) #0
```
\
...
nrow_Hyp_z_uneq <- nrow(Hyp_z_uneq) #object with number of columns subset
...
#Numbers of (decision) errors.
sum(as.numeric(Hyp_z_uneq$Error)) #10, both OneSided == 0 and OneSided == 2
sum(as.numeric(Hyp_z_uneq$DecisionError)) #1
```
\
...
#Calculating p-values belonging to the lower and upper bounds of acceptable z-values. We performed a one-sided test on a z-statistic if a one-tailed test was done, and a two-sided test if a two-tailed test was done or if sidedness of the test was unknown.
...
#Numbers of (decision) errors.
sum(as.numeric(Hyp_z_eq$Error)) #2, concern results for which OneSided == 0
sum(as.numeric(Hyp_z_eq$DecisionError)) #0
```
\
\

### *4.3.4 Recalculating p-values based on $\chi^2$-values* 

*Among reported p-values based on $\chi^2$-values, 9 (decision) errors were found, see below.*
...
Hyp_chi$OneTailed <- as.numeric(as.character(Hyp_chi$OneTailed)) #column with information on sidedness tests, type 'numeric'
Hyp_chi$t_stat_abs <- abs(Hyp_chi$t_stat) #column containing absolute chi^2-values
nrow_Hyp_chi <- nrow(Hyp_chi) #object with number of columns above-mentioned subset
nrow(Hyp_chi) #n = 64

#Performing a one-sided test if a one-tailed test was done, and a two-sided test if a two-tailed test was done or if sidedness of the test was unknown
...
For results of which the sidedness was unknown, two-sided testing was initially done. There were some results for which this resulted in recalculated *p* > 1. We applied one-sided testing to these results, to see if their *p*-values actually seemed correct under one-sided testing.
```{r changing sidedness of results chi^2, warning=FALSE, message = FALSE, echo = TRUE, results = 'hide'}
...
#Recalculating result for which Computed > 1. 
for (i in 1:nrow_Hyp_chi){
...
#Numbers of (decision) errors.
sum(as.numeric(Hyp_chi_ns$Error)) #n = 0
sum(as.numeric(Hyp_chi_ns$DecisionError)) #n = 0
```
\
...
Hyp_chi_sml <- subset(Hyp_chi, Reported.Comparison == 1) #subset of results with Reported.Comparison '<'
nrow_Hyp_chi_sml <- nrow(Hyp_chi_sml) #numeric object for subset's number of columns 
...
#Numbers of (decision) errors.
sum(as.numeric(Hyp_chi_sml$Error)) #8, both OneSided == 0 and OneSided == 2
sum(as.numeric(Hyp_chi_sml$DecisionError)) #2
```
\
...
Hyp_chi_eq <- subset(Hyp_chi, Reported.Comparison == 4) #subset results with exactly reported p-values
nrow(Hyp_chi_eq) #n = 1
nrow_Hyp_chi_eq = nrow(Hyp_chi_eq) #numeric object for subset's number of columns 
options(scipen=999)
...
#Numbers of (decision) errors.
sum(as.numeric(Hyp_chi_eq$Error)) #1, concerns a result for which OneSided == 0
sum(as.numeric(Hyp_chi_eq$DecisionError)) #1
```
\
...
Hyp_chi_lrg <- subset(Hyp_chi, Reported.Comparison == 5) #subset of results with reported comparison '<'
nrow_Hyp_chi_lrg <- nrow(Hyp_chi_lrg) #numeric object for number of columns of subset
...
#Numbers of (decision) errors.
sum(as.numeric(Hyp_chi_lrg$Error)) #0
sum(as.numeric(Hyp_chi_lrg$DecisionError)) #0
```
\
\

### *4.3.5 Recalculating p-values based on F-values*

*Among reported p-values based on F-values, 11 (decision) errors were found, see below.*
...
nrow_Hyp_f <- nrow(Hyp_f) #object number of columns above-mentioned subset
nrow(Hyp_f) #n = 16
...
For results of which the sidedness was unknown, two-sided testing was initially used. There was one result for which this lead to recalculated *p* > 1. We applied one-sided testing to this result to see if its *p*-value seemed correct under one-sided testing. 
```{r changing sidedness of certain results, warning=FALSE, message = FALSE, echo = TRUE, results = 'hide'}
#Changing results to one-tailed when Computed > 1.
Hyp_f$OneTailed[which(Hyp_f$OneTailed== 0 &  Hyp_f$Computed > 1)] <- "1" 
...
Hyp_f_uneq <- subset(Hyp_f, Reported.Comparison == 1) #subset of results with Reported.Comparison '<'
nrow_Hyp_f_uneq <- nrow(Hyp_f_uneq) #numeric object for number of columns of subset
nrow(Hyp_f_uneq) #n = 3
...
#Numbers of (decision) errors.
sum(as.numeric(Hyp_f_uneq$Error)) #0
sum(as.numeric(Hyp_f_uneq$DecisionError)) #0
```
\
...
nrow_Hyp_f_eq = nrow(Hyp_f_eq) #numeric object number of columns 
options(scipen=999)
...
for (i in 1:nrow_Hyp_f_eq){
  Hyp_f_eq$dec_pval_rep[i] <- decimalplaces(Hyp_f_eq$Reported.P.Value[i])
    if (Hyp_f_eq$OneTailed[i] == 1) {Hyp_f_eq$Comp_ub[i] <- (1 - pf(Hyp_f_eq$ub_stat_abs[i], Hyp_f_eq$df_1[i], Hyp_f_eq$df_2[i]))} else {Hyp_f_eq$Comp_ub[i] <- (2*(1 - pf(Hyp_f_eq$ub_stat_abs[i], Hyp_f_eq$df_1[i], Hyp_f_eq$df_2[i])))}
    if (Hyp_f_eq$OneTailed[i] == 1) {Hyp_f_eq$Comp_lb[i] <- (1 - pf(Hyp_f_eq$lb_stat_abs[i], Hyp_f_eq$df_1[i], Hyp_f_eq$df_2[i]))} else {Hyp_f_eq$Comp_lb[i] <- (2*(1 - pf(Hyp_f_eq$lb_stat_abs[i], Hyp_f_eq$df_1[i],  Hyp_f_eq$df_2[i])))}
...
#Numbers of (decision) errors.
sum(as.numeric(Hyp_f_eq$Error)) #11, concerns results for which OneSided == 0
sum(as.numeric(Hyp_f_eq$DecisionError)) #2 
```
\
\

### *4.3.6 Merging data sets* 
...
Merging data sets containing results with reported *r*-values.
...
Merging data sets containing results with reported *z*-values.
...
#Merging (1) data set with results that have reported comparison 'ns' and inexactly reported p-values with (2) data set with results with exactly reported p-values.
Hyp_z_mrgd <- merge(Hyp_z_ns_uneq, Hyp_z_eq, all = TRUE) #merging 
nrow(Hyp_z_mrgd) #n = 262
sum(as.numeric(Hyp_z_ns_uneq$Error)) #10
sum(as.numeric(Hyp_z_ns_uneq$DecisionError)) #1
```
\

Merging data sets containing results with reported $\chi^2$-values.
...
sum(as.numeric(Hyp_chi_mrgd$Error)) #9
sum(as.numeric(Hyp_chi_mrgd$DecisionError)) #3
```
\
...
sum(as.numeric(Hyp_f_mrgd$Error)) #11
sum(as.numeric(Hyp_f_mrgd$DecisionError)) #2
```
\
...
Hyp_unkn_corr <- subset(Hyp_fin, OneTailed != 0 | (Error != 1 & DecisionError != 1)) 
nrow(Hyp_unkn_corr) #n = 377
#Selecting results that were possibly deemed erroneous because we incorrectly assumed they were two-tailed.
Hyp_unkn_err <- subset(Hyp_fin, OneTailed == 0 & (Error == 1 | DecisionError == 1)) 
nrow(Hyp_unkn_err) #n = 27

#We take into consideration the possibility that p-values might be correct when we assume they are one-tailed rather than two-tailed. 
Hyp_unkn_err$OneTailed[which(Hyp_unkn_err$OneTailed=="0")] <- "1" #making testing erroneous results to one-sided 
nrow_Hyp_unkn_err <- nrow(Hyp_unkn_err) #numeric object that represents subset's number of columns 

#Performing one-sided tests on the different test statistics in the data set with results that were erroneous under the assumption of two-sided testing.
for (i in 1:nrow_Hyp_unkn_err){
  if (Hyp_unkn_err$Statistic[i] == 4) {Hyp_unkn_err$P_one_sided[i] <- (1 - pnorm(Hyp_unkn_err$t_stat_abs[i]))} #z-values
  if (Hyp_unkn_err$Statistic[i] == 7) {Hyp_unkn_err$P_one_sided[i] <- (1 - pchisq(Hyp_unkn_err$t_stat_abs[i], df = Hyp_unkn_err$df_1[i]))} #chi^2-values
  if (Hyp_unkn_err$Statistic[i] == 8) {Hyp_unkn_err$P_one_sided[i] <-  (1 - pf(Hyp_unkn_err$t_stat_abs[i], Hyp_unkn_err$df_1[i], Hyp_unkn_err$df_2[i]))}} #F-values
```
\
...
nrow_Hyp_unkn_err_sml <- nrow(Hyp_unkn_err_sml) #object number of rows subset

#Error if Reported.P.Value < P_one_sided.
Hyp_unkn_err_sml <- Hyp_unkn_err_sml %>% mutate(Error = ifelse(Reported.P.Value >= P_one_sided, 0, 1))
#DecisionError if Reported.P.Value <= .05 and Computed > .05.
Hyp_unkn_err_sml <- Hyp_unkn_err_sml %>% mutate(DecisionError = ifelse(Reported.P.Value <= .05 & P_one_sided > .05, 1, 0)) 
...
nrow_Hyp_unkn_err_lrg <- nrow(Hyp_unkn_err_lrg) #object number of rows subset

#Error if Reported.P.Value > P_one_sided.
Hyp_unkn_err_lrg <- Hyp_unkn_err_lrg %>% mutate(Error = ifelse(Reported.P.Value <= P_one_sided, 0, 1))
#DecisionError if Reported.P.Value >= .05 and Computed < .05.
Hyp_unkn_err_lrg <- Hyp_unkn_err_lrg %>% mutate(DecisionError = ifelse(Reported.P.Value >= .05 & P_one_sided < .05, 1, 0)) 

#Subset of results with p-values reported as 'ns'.
Hyp_unkn_err_ns <- subset(Hyp_unkn_err, Reported.Comparison == 3) 

#Since a (decision) error occurs if a result that was reported as non-significant is significant, a "1" is assigned if Computed <= .05, and a "0" is assigned if Computed > .05.
Hyp_unkn_err_ns$Error[which(Hyp_unkn_err_ns$P_one_sided > 0.05)] <- "0"
Hyp_unkn_err_ns$DecisionError[which(Hyp_unkn_err_ns$P_one_sided > 0.05)] <- "0"
Hyp_unkn_err_ns$Error[which(Hyp_unkn_err_ns$P_one_sided <= 0.05)] <- "1"
Hyp_unkn_err_ns$DecisionError[which(Hyp_unkn_err_ns$P_one_sided <= 0.05)] <- "1"
```
\
...
Hyp_unkn_err_eq <- subset(Hyp_unkn_err, Reported.Comparison == 4) #subset of results with exactly reported p-values
nrow_Hyp_unkn_err_eq = nrow(Hyp_unkn_err_eq) #object number of rows subset

options(scipen=999)
#Calculating the p-values of the lower bounds of acceptable test statistic values. 
for (i in 1:nrow_Hyp_unkn_err_eq){
  if (Hyp_unkn_err_eq$Statistic[i] == 4) {Hyp_unkn_err_eq$Comp_lb[i] <- (1 - pnorm(Hyp_unkn_err_eq$lb_stat_abs[i]))} #z-values
  if (Hyp_unkn_err_eq$Statistic[i] == 7) {Hyp_unkn_err_eq$Comp_lb[i]  <- (1 - pchisq(Hyp_unkn_err_eq$lb_stat_abs[i], df = Hyp_unkn_err_eq$df_1[i]))} #chi^2-values
  if (Hyp_unkn_err_eq$Statistic[i] == 8) {Hyp_unkn_err_eq$Comp_lb[i] <-  (1 - pf(Hyp_unkn_err_eq$lb_stat_abs[i], Hyp_unkn_err_eq$df_1[i], Hyp_unkn_err_eq$df_2[i]))}} #F-values

#Calculating the p-values of the upper bounds of acceptable test statistic values. 
for (i in 1:nrow_Hyp_unkn_err_eq){
  if (Hyp_unkn_err_eq$Statistic[i] == 4) {Hyp_unkn_err_eq$Comp_ub[i] <- (1 - pnorm(Hyp_unkn_err_eq$ub_stat_abs[i]))} #z-values
  if (Hyp_unkn_err_eq$Statistic[i] == 7) {Hyp_unkn_err_eq$Comp_ub[i]  <- (1 - pchisq(Hyp_unkn_err_eq$ub_stat_abs[i], df = Hyp_unkn_err_eq$df_1[i]))} #chi^2-values
  if (Hyp_unkn_err_eq$Statistic[i] == 8) {Hyp_unkn_err_eq$Comp_ub[i] <-  (1 - pf(Hyp_unkn_err_eq$ub_stat_abs[i], Hyp_unkn_err_eq$df_1[i], Hyp_unkn_err_eq$df_2[i]))}} #F-values
...
#DecisionError if Reported.P.Value <= .05 and Computed > .05, or vice versa.
Hyp_unkn_err_eq <- Hyp_unkn_err_eq %>% mutate(DecisionError = ifelse((Reported.P.Value <= 0.05 & P_one_sided > 0.05)|(Reported.P.Value > 0.05 & P_one_sided <= 0.05), 1, 0)) 
...
Hyp_all_err_n <- nrow(Hyp_all_err)
...
### *4.3.8 Descriptive information errors*

#### *4.3.8.1 Article level*
Tables on articles with errors per category and in total.
...
#### *4.3.8.3 Final tables on statistical reporting errors* 
Tables on errors for each category and for results in total.
...
### *4.5.2 Descriptive information p-value range (.01-.05]*
Creating a new data frame with *p*-values in the range (.01-.05].
...
### *4.5.3 Final tables bump in p-values*
...
### *4.6.1 Preliminary calculations*
Firstly, we calculate some basic statistics relevant to marginal significance at the article and results level. These will be used to calculate other statistics later on. Results that are reported using the signs '>' or '$\ge$' are not taken into account, since these do not imply results were near the conventional significance level, i.e., *p* = .05. 
```{r some basic statistics marginal significance (Hyp), warning=FALSE, message = FALSE, echo = TRUE, results = 'hide'}
#Selecting relevant data.
Hyp_marg <- subset(Hyp, Reported.P.Value > .05 & Reported.P.Value <= .1 & Reported.Comparison != 2 & Reported.Comparison != 5) #subsetting the data, no results with '>' or '>='   
Hyp_marg_n <- nrow(Hyp_marg) 
Hyp_marg_n #n = 130 results with p-values in range (.05-.10]

#Subset marginally significant results.
Hyp_marg_1 <- subset(Hyp_marg, Marg.sig == 1) 
Hyp_marg_1_n <- nrow(Hyp_marg_1) 
Hyp_marg_1_n #n = 106 results reported as marginally significant

#Removing duplicates to count number of articles.
Hyp_marg_nd <- Hyp_marg[!duplicated(Hyp_marg$Article_numb), ] 
Hyp_marg_nd_nrow <- nrow(Hyp_marg_nd) 
Hyp_marg_nd_nrow #n = 30 articles with p-values in range (.05-.10]
Hyp_marg_perc <- round(((Hyp_marg_1_n/Hyp_marg_n)*100), digits = 1) #percentage
...
Hyp_marg_nd_perc <- (Hyp_marg_1_nd_nrow/Hyp_marg_nd_nrow)*100 #percentage

#Subset results that are reported as not marginally significant.
Hyp_marg_0 <- subset(Hyp_marg, Marg.sig == 0)
Hyp_marg_0_n <- nrow(Hyp_marg_0) 
Hyp_marg_0_n #n = 24

#Articles with no p-values in the range (.05-.10] that are reported as marginally significant.
Hyp_marg_0_nd_nrow <- Hyp_marg_nd_nrow - Hyp_marg_1_nd_nrow 
Hyp_marg_0_nd_nrow #n = 19
```
\
\

### *4.6.2 Descriptive information article level*
Creating tables containing information on marginal significance at the article level. Data for journals and years are included.
...
### *4.6.3 Descriptive information tables results level*
Creating tables containing information on marginal significance at the results level. Data for journals and years are included.
...
#Per journal
jrnl_Hyp_marg_tot <- as.matrix(table(Hyp_marg$Journal)) #number of results with p-values in the range (.05-.10) per journal
...
jrnl_Hyp_marg <- cbind(jrnl_Hyp_marg_tot, jrnl_Hyp_marg_0, jrnl_Hyp_marg_1) #binding together results
jrnl_Hyp_marg <- as.data.frame(jrnl_Hyp_marg) 
jrnl_Hyp_marg <- jrnl_Hyp_marg %>% #calculating percentages results reported as marginally significant
...
#Per year
yr_Hyp_marg_tot <- as.matrix(table(Hyp_marg$Year)) #number of results with p-values in the range (.05-.10) per year
yr_Hyp_marg_1 <- as.matrix(table(Hyp_marg_1$Year)) #number of results reported as marginally significant per year
yr_Hyp_marg_0 <- as.matrix(table(Hyp_marg_0$Year)) #number of results not reported as marginally significant per year
yr_Hyp_marg <- cbind(yr_Hyp_marg_tot, yr_Hyp_marg_0, yr_Hyp_marg_1) #binding together results
yr_Hyp_marg <- as.data.frame(yr_Hyp_marg) #creating a data frame
yr_Hyp_marg <- yr_Hyp_marg %>% #calculating percentages results reported as marginally significant
...
### *4.6.4 Difference marginally significant results 'AllP' and 'Hyp'*

We conducted a *z*-test for 2 independent proportions using the following formula:

$$ z = \frac{dif}{SE_0} = \frac{p_1 - p_2 = \hat{p}}{\hat{p}\cdot(1 - \hat{p})\cdot (\frac{1}{n_1}+\frac{1}{n_2})}, $$ where $p_1$ and $p_2$ denote the proportions of results to which marginal significance was assigned in the 'Hyp' and 'AllP' data sets, respectively, and $n_1$ and $n_2$ denote the total amounts of *p*-values in the range (.05-.10] for the 'Hyp' and 'AllP' data sets, respectively.
\   

The percentage of *p*-values in interval (.05-.10] reported as marginally significant is much higher for manually retrieved statistical results related to explicitly stated hypotheses in ‘Hyp’ (81.5%) than for reported *p*-values in ‘AllP’ (35%), *z* = 8.33, *p* < .001.

```{r calculation difference marginally significant results, warning=FALSE, message = FALSE}
p1 <- (106/130) #proportion marginally significant results 'Hyp'
p2 <- (72/206) #proportion marginally significant results 'AllP'
p <- ((106/130)-(72/206)) #p1 - p2
z <- (p1-p2)/sqrt(p*(1-p)*((1/130)+(1/206))) #calculate z
z #z = 8.33
2*(1 - pnorm(z)) #p < .001
```
\
...
### *4.6.5 Final tables on marginal significance * 
