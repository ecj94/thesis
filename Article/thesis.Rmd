...
data_table3 <- c("ns", "Cal\\emph{P} $\\leq$ .05", "Cal\\emph{P} $\\leq$ .05", "$<$", "Cal\\emph{P} $\\geq$ Rep\\emph{P}", "Cal\\emph{P} $>$ .05 \\& Rep\\emph{P} $\\leq$ .05", "$\\geq$", "Cal\\emph{P} $<$ Rep\\emph{P}", "Cal\\emph{P} $<$ .05 \\& Rep\\emph{P} $\\geq$ .05", "=", " Cal\\emph{P} $\\neq$ Rep\\emph{P} not due to", "Cal\\emph{P} $\\neq$ Rep\\emph{P} not due to rounding,", "", "rounding\\textsuperscript{*}", "Cal\\emph{P} $\\leq$ .05 \\& Rep\\emph{P} $>$ .05 or vice versa")
#Putting content table cells into a matrix.
...
```{r tableS1 inconsistencies and gross inconsistencies, echo = FALSE, warning = FALSE, message = FALSE, include=knitr::is_latex_output()}
...
hyp_table_err <- c("-1.820", ".157", "< .001", ".162 [.118, .218]", #intercept H1
                 "-0.073", ".278", ".793", ".930 [.531, 1.585]", #relation hypothesis H1
                 "-4.422", ".503", "< .001", ".012 [.004, .028]", #intercept H2
                 ".708", ".714", ".321", "2.030 [.475, 8.685]") #relation hypothesis H1
#Putting content table cells into a matrix.
...
```{r tableS2 publication bias, echo = FALSE, warning = FALSE, message = FALSE, include=knitr::is_latex_output()}
#Creating content table cells.
hyp_table_pb <- c("29", "168", "22 (13.1%)", "4 (2.4%)", 
                 "68", "337", "47 (13.9%)", "4 (1.2%)")
#Putting content table cells into a matrix.
hyp_table_pb <- matrix(hyp_table_err, ncol = 4, nrow = 2, byrow = TRUE)

#Giving matrix row and column names.
colnames(hyp_table_pb) <- c("b", "SE", "p", "OR [95% CI]")
rownames(hyp_table_pb) <- paste(c("Intercept", "Result hypothesis"))

#Creating Table S2.
hyp_table_pb %>%
  #format html, centered alignment, bold-faced caption
kable(booktabs = T, format ="latex", align = "c", caption = "Logistic regression for H3, which concerns the difference in prevalence of publication bias in results related to hypotheses versus results not related to hypotheses.") %>% 
  #table will appear in correspondence with its place in this document, table is scaled-down to fit page size
#hyp_table_err %>%
  kable_styling(latex_options = c("HOLD_position", "striped"), full_width = F)
```
